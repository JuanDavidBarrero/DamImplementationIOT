...
export const getDeviceData = async (req = request, res = response) => {
    ...
    try {
        ...
        const limitQuery = unique === '1' ? 'ORDER BY l.date DESC LIMIT 1' : '';  
        ...
        const [deviceData] = await connection.execute(`
            ...
            LEFT JOIN Logs l ON d.valve_id = l.valve_id
            WHERE d.device_id = ?
            ${limitQuery}`, [id]);
        ...
        const processedDeviceData = deviceData.map(data => {
            return {
                ...data,
                logs: data.logs === 'open' ? true : false
            };
        });

        return res.status(200).json(processedDeviceData);
